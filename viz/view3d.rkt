...
; deleted.
(define (load-program node preamble-port)
  (when program
    (delete-program)
    (set! program #f)
  ...
  (let* ([program (glCreateProgram)]
         ...
         [lines (load-program-source shader node preamble-port)])
...
(define (load-program-source shader node preamble-port)
  ...
  (let* ([preamble (for/vector ([line (in-lines preamble-port)])
                     (string-append line "\n"))]
         [gen (for/vector ([line (in-list (generate-glsl-and-textures node))])
                (string-append line "\n"))]
...
; metadata texture(s).
(define (generate-glsl-and-textures node)
  ...
  (if node
    ...
    (if (or (gl-version-at-least? '(2 0))
      ...
      (set!-values (program delete-program)
                   (call-with-input-file preamble-glsl
                                         (lambda (f) (load-program node f))))
