...
; This is a direct gloss of Paul Bourke's implementation in C, which I believe
; to be in the public domain: http://paulbourke.net/geometry/polygonise/
                  ...
                  (if (zero? (et . bitwise-and . (1 . arithmetic-shift . n)))
                    ...
                    (let ([start (n . modulo . 8)]
                      ...
                      (vec3-linear-root (g-point-ref g start)
                                        ...
                                        (g-point-ref g end)
                                        (g-value-ref g end))))))])
        (for ([i (in-range 0 (vector-length tt) 3)])
          (out-fn (list (vector-ref verts (vector-ref tt i))
                        (vector-ref verts (vector-ref tt (+ i 2)))
                        (vector-ref verts (vector-ref tt (+ i 1)))
                        )))))))
